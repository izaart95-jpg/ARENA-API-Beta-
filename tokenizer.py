...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .windows { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px; }
...
async function refresh() {
  try {
    ...
    const v2 = allTokens.filter(t => t.version === 'v2').length;
    ...
    // Attach per-window token counts
    const countByWindow = {};
...
async def store_token(request: Request):
    ...
    async with _tokens_lock:
        # Tag with window id derived from source_url or just unknown
...
async def setup_window(playwright, window_id: int):
    """Launch browser/tab, navigate to arena.ai, signal ready, start mouse mover."""
...
async def tab_switcher():
    """
    TABS mode only — cycles through all tabs every 3 seconds.
...
async def run_browsers(server_ready_event: asyncio.Event):
    ...
    async with async_playwright() as pw:
        ...
        if TABS:
            ...
            print("   Tab switcher active (cycles every 3s to prevent throttling)")
